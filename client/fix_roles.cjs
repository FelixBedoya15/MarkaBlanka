...
async function fixRoles() {
    ...
    console.log('Updating ADMIN role SGSST permissions to true...');
    const resultAdmin = await Role.updateOne(
        { name: 'ADMIN' },
        {
            $set: { 'permissions.SGSST.USE': true }
        }
    );
    console.log('ADMIN update result:', resultAdmin);
